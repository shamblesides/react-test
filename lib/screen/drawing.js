...
export const sprite = (sheet, sprite, flip=null, colors=null, { left:cropLeft=0, right:cropRight=0, top:cropTop=0, bottom:cropBottom=0 }={}) => {
    return {
        at: (x, y) => ({
            ready() {
...
export const multi = (width, height, sprites) => {
    const ready = memoize(function ready() {
        const promises = sprites.reduce((set, s) => set.add(s.ready()), new Set());
