...
const sheetRgbToIndexMaps = new Map();
...
function createSpriteCanvas({ name, spriteWidth=null, spriteHeight=null }, sprite=0) {
...
function makeRgbToIndexMap(sheetName) {
    const imagedata = sheetImagedatas.get(sheetName);
...
export function getImage({ sheet, sprite=0, flip='', colors=null }) {
    ...
    const rgbToIndexMap = sheetRgbToIndexMaps.get(sheet.name) || makeRgbToIndexMap(sheet.name);
    if (!sheetRgbToIndexMaps.has(sheet.name)) sheetRgbToIndexMaps.set(sheet.name, rgbToIndexMap);
    // color it
