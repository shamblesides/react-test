...
function getScaledSprite({ name,spriteWidth=null,spriteHeight=null }, sprite, scale) {
...
export function getImage({ sheet,sprite=0,scale }) {
    const key = getKey({ sheet, sprite, scale });

    if (spriteImages.has(key)) return spriteImages.get(key);

    const res = getScaledSprite(sheet, sprite, scale);
    spriteImages.set(key, res);
    return res;
}
