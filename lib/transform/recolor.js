...
const getRgbToIndexMap = memoize(function getRgbToIndexMap(canvas) {
    ...
    return [...imgColors]
        .map((rgba) => {
            const brightness = (rgba & 0xff) + (rgba >> 8 & 0xff) + (rgba >> 16 & 0xff);
            return [rgba, brightness];
        }).sort(([, b1], [, b2]) => b1 - b2)
        .reduce((map, [rgba], i) => map.set(rgba, i), new Map());
});
...
export function recolor(colors) {
    return (canvas, sheetCanvas) => recolorCanvas(canvas, getRgbToIndexMap(sheetCanvas), colors);
}