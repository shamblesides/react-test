...
const getRgbToIndexMap = memoize(function getRgbToIndexMap(sheetImg) {
    const sheetImagedata = getImagedataFromImg(sheetImg);
...
export function recolor(colors) {
    return (canvas, sheetImg) => recolorCanvas(canvas, getRgbToIndexMap(sheetImg), colors);
}